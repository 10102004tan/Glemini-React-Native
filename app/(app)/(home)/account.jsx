   ...
   const handleNotification = (item) => {
      ...
      switch (item.noti_type) {
         ...
         case SHARE_QUIZ_TO_CLASSROOM:
            break;
         case ROOM_REALTIME:
            setCurrentRoom(item.noti_options.room_code);
            socket.emit('joinRoom', { roomCode: item.noti_options.room_code, user: userData });
            router.replace({
               pathname: '/(app)/(teacher)/teacher_room_wait',
               params: { roomCode: item.noti_options.room_code }
            });
            break;
