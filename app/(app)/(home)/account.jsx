...
import {SYSTEM_MAINTENANCE, SYSTEM_NEW_FEATURE,SHARE_QUIZ_TO_TEACHER,SHARE_QUIZ_TO_CLASSROOM,ROOM_REALTIME} from "@/utils/notificationCode";
...
export default function AccountScreen() {
    const {userData:{user_fullname,user_email,user_type,user_avatar},userData,teacherStatus,notification,updateNotificationStatus,setNumberOfUnreadNoti,setNotification} = useContext(AuthContext);
        ...
        <View className="bg-white h-[100%] px-[20px]">
            ...
            <View className={"mb-3"}>
                ...
                <FlatList data={notification.slice(0,COUNT_NOTIFICATION_RECENT)} renderItem={(item)=>{
                    ...
                    return (
                        <NotificationCard status={noti_status} onPress={()=>handleNotification(item.item)} type={noti_type} content={content}
                                          time={createdAt} options={noti_options}/>
                    )
                }}/>
            </View>

            <Modalize onClosed={onClosed} avoidKeyboardLikeIOS={true} children={<View></View>} modalStyle={{padding: 10, marginTop: 30,zIndex:1000}}  ref={modalizeRef}
