...
const Library = () => {
  ...
  const [visibleCreateNewBottomSheet, setVisibleCreateNewBottomSheet] = useState(false);

  const [visibleFilterBottomSheet, setVisibleFilterBottomSheet] = useState(false);
  ...
  const getAllQuizzesShared = async () => {
    if (!quizLoading) {
      ...
      const path = `${API_VERSION.V1}${END_POINTS.GET_ALL_QUIZZES_SHARED}`;
  ...
  const handleDeleteQuizShared = (quiz_id) => {
    Alert.alert('Xác nhận xóa', 'Bạn có chắc chắn muốn xóa quiz được chia sẻ này không?', [
  ...
  const handleTabChange = (tab) => {
    ...
    Animated.timing(translateValue, {
      toValue:
        tab === 'library' ? 0 : tab === 'collection' ? screenWidth / 3 : (2 * screenWidth) / 3,
      duration: 300,
  ...
  const resetFilters = async () => {
    ...
    const path = `${API_VERSION.V1}${END_POINTS.QUIZ_FILTER}`;
    const response = await api.post(path, body);
    const data = response.data;
    // console.log(data)
    if (data.statusCode === 200) {
