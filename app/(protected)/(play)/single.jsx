...
const SinglePlay = () => {
  ...
  useEffect(() => {
    if (result && result._id) {
      ...
      const previousScore =
        result.result_questions?.reduce((total, question) => {
  ...
  if (state.isCompleted && resultId) {
    return <ResultSingle resultId={resultId} handleRestart={handleRestart} />;
  }
          ...
          {isLoading ? (
            ...
            questions[state.currentQuestionIndex]?.question_answer_ids.map((answer, index) => {
              ...
              if (state.showCorrectAnswer) {
                const correctIds = questions[state.currentQuestionIndex].correct_answer_ids.map(
                  (a) => a._id,
                );
