...
const EditQuizQuestion = () => {
  ...
  const [pointBotttomSheetVisible, setPointBotttomSheetVisible] = useState(false); // bottom sheet để chọn số điểm
  const [timeBotttomSheetVisible, setTimeBotttomSheetVisible] = useState(false); // bottom sheet để chọn số điểm
  ...
  return (
    <Wrapper>
      ...
      <Overlay
        ...
        visible={showQuestionBoard || pointBotttomSheetVisible || timeBotttomSheetVisible}
      />
      {/* Bottom Sheet Point */}
      <BottomSheet
      ...
      <View className="flex flex-row items-center justify-start p-4">
        ...
        >
          <Text className="mr-2">
            {selectedTime} {i18n.t('edit_quiz_screen.second')}
          </Text>
        ...
        >
          <Text className="mr-2">
            {selectedPoint} {i18n.t('edit_quiz_screen.point')}
          </Text>
        ...
        <ScrollView>
          ...
          <View className="flex items-center justify-between mt-4 flex-row">
            {(question.question_type === 'single' || question.question_type === 'multiple') && (
              <TouchableOpacity
                onPress={() => {
                  if (question.question_type === 'box' || question.question_type === 'blank') {
                    return;
                  }
                  handleTypeQuestion();
                }}
                className="flex items-center justify-center flex-row bg-overlay py-2 px-4 rounded-xl"
                style={mutipleChoice ? { backgroundColor: '#0BCA5E' } : {}}
              >
                <Text className="text-white">{i18n.t('edit_quiz_screen.mutipleChoice')}</Text>
              </TouchableOpacity>
            )}
            <TouchableOpacity
              className="flex items-center justify-center flex-row bg-overlay py-2 px-4 rounded-xl"
          ...
          <View className="flex items-center justify-between mt-4 flex-row">
            {(question.question_type === 'single' || question.question_type === 'multiple') && (
              <TouchableOpacity
                className="flex items-center justify-center flex-row bg-overlay py-2 px-4 rounded-xl"
                onPress={() => {
                  if (question.question_answer_ids.length < MAX_ANSWER) {
                    addAnswer();
                  } else {
                    // Alert to user here
                  }
              ...
              >
                <Text className="text-white">{i18n.t('edit_quiz_screen.addAnswer')}</Text>
              </TouchableOpacity>
            )}
            <TouchableOpacity
              className="flex items-center justify-center flex-row bg-overlay py-2 px-4 rounded-xl"
            ...
            >
              <Text className="text-white">{i18n.t('edit_quiz_screen.deleteAnswer')}</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </View>
