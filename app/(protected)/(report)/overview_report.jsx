      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1 p-5">
        {overViewData.result_questions && overViewData.result_questions?.length > 0 && (
          <View className="flex-row h-6 rounded-full overflow-hidden mb-2">
            <View
              style={{ flex: correctCount / overViewData.result_questions?.length }}
              className="bg-green-500"
            />
            <View
              style={{ flex: incorrectCount / overViewData.result_questions?.length }}
            ...
            >
              <View className="flex-row justify-between items-center mb-2">
                <View className="flex-row justify-start gap-2">
                  ...
                  <View
                    className={`px-3 py-2 rounded-lg ${question.correct ? 'bg-green-500' : 'bg-red-500'}`}
                  >
                    <Text className="text-white font-semibold text-sm w-12 text-center">
                      {question.correct
              ...
              <Text className="font-medium text-base pb-4">
                {question.question_id.question_excerpt}
              </Text>
              <Text className="text-base mt-2 font-semibold ">{`${overViewData.user_id?.user_fullname}`}</Text>
              <Text
                className={`${question.correct ? 'text-green-500' : 'text-red-500'} font-medium border-b-[1px] pb-4 ${question.correct ? 'border-green-500' : 'border-red-500'}`}
              >
