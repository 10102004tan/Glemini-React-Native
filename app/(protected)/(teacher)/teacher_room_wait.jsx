...
const TeacherRoomWaitScreen = () => {
  ...
  useEffect(() => {
    const getRoomData = async () => {
      const response = await fetch(`${API_URL}${API_VERSION.V1}${END_POINTS.ROOM_DETAIL}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-client-id': userData._id,
    ...
    <Wrapper>
        ...
        <View className="bg-white p-4 rounded-2xl">
          ...
          <ScrollView showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false}>
            ...
            <Button
              ...
              otherStyles="p-3 mt-4 w-full flex items-center justify-center bg-[#A1732A]"
              textStyles=""
            />
          </ScrollView>
        </View>
      ...
      <View className="flex-1 p-4 bg-primary pt-[60px]">
        <View className="flex flex-row items-center justify-end w-full">
          <Button
            text={i18n.t('room_wait.leave')}
            otherStyles="p-3 bg-red-500 justify-center w-fit"
            onPress={() => {
              Alert.alert(i18n.t('room_wait.alert'), i18n.t('room_wait.exitRoomConfirmation'), [
                {
        ...
        <View className="my-4">
          {roomData && roomData.status === 'completed' && (
            <Text className="text-white bg-red-500 text-center p-4 rounded-2xl">
              {i18n.t('room_wait.roomAlreadyCompleted')}
            </Text>
          )}
          {roomData && roomData.status === 'doing' && (
            <Text className="text-white bg-red-500 text-center p-4 rounded-2xl">
              {i18n.t('room_wait.roomInProgress')}
            </Text>
          )}
        </View>
        <ScrollView
        ...
        >
          ...
          <View className="p-4 rounded-2xl bg-[#2f3542] w-full">
            <View className="w-full h-[200px] bg-[rgba(117, 117, 117, 0.3)] flex items-center justify-center p-4 rounded-2xl">
              ...
              {userData &&
                roomData &&
                userData._id === roomData.user_created_id &&
                roomData.status === 'completed' && (
                  <Button
                    text={i18n.t('room_wait.openAgain')}
                    onPress={() => {
                      handleReOpenRoom();
                    }}
                    otherStyles="p-3 mt-4 w-full flex items-center justify-center bg-[#A1732A]"
              ...
              {userData &&
                roomData &&
                userData._id === roomData.user_created_id &&
                roomData.status === 'doing' && (
                  <Button
                    text={i18n.t('room_wait.followResult')}
                    onPress={() => {
                      router.replace({
                        pathname: '/(teacher)/teacher_room_wait_result',
                        params: {
          ...
          {userData && roomData && userData._id === roomData.user_created_id && (
            <View className="mt-4 bg-[#2f3542] p-8 flex items-center justify-center rounded-2xl">
          ...
          <View className="mt-4 p-4 rounded-2xl bg-[#2f3542] w-full">
            <Text className="text-white text-center">
              {i18n.t('room_wait.totalJoined')} (
              {totalJoindUsers > 0 ? totalJoindUsers - 1 : totalJoindUsers})
            </Text>
          </View>
