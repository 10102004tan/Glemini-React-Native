  ...
  const fetchUrls = async () => {
    try {
      const response = await fetch(
        `${API_URL}${API_VERSION.V1}${END_POINTS.PROFILE_TEACHER_IMAGES}`,
        {
          method: 'POST',
          headers: {
  ...
  return (
    <View className={'mt-3 mx-3'}>
      ...
      {teacherStatus === 'rejected' && (
        <View className={'bg-amber-200 rounded w-[100%] mb-3 p-3'} style={{ height: 100 }}>
          <FontAwesome name={'exclamation-circle'} size={24} color={'#f00'} />
          <Text className={'text-gray mt-2'}>{i18n.t('signUp.rejection')}</Text>
        </View>
      )}
      ...
      {teacherStatus === 'pedding' || teacherStatus === 'active' ? (
        ...
        <View>
          <InputImage
            onLongPress={() => handlerLongPress({ type: TYPEIMAGE.Card })}
            onPress={() => {
              handlerPickImage(TYPEIMAGE.Card);
            }}
            desc={i18n.t('signUp.descForCard')}
            title={i18n.t('signUp.card')}
            logo={
              imageCard ? imageCard.uri : 'https://cdn-icons-png.flaticon.com/512/175/175062.png'
            }
          />
          <InputImage
            onLongPress={() => handlerLongPress({ type: TYPEIMAGE.IDCard })}
            onPress={() => {
              handlerPickImage(TYPEIMAGE.IDCard);
            }}
            desc={i18n.t('signUp.descForCardID')}
            title={i18n.t('signUp.cardID')}
            logo={
              imageIDCard
                ? imageIDCard.uri
          ...
          />
          <InputImage
            onLongPress={() => handlerLongPress({ type: TYPEIMAGE.Confirm })}
            onPress={() => {
              handlerPickImage(TYPEIMAGE.Confirm);
            }}
            desc={i18n.t('signUp.descForDocument')}
            title={i18n.t('signUp.documentConfirm')}
            logo={
              imageConfirm
                ? imageConfirm.uri
                : 'https://cdn-icons-png.freepik.com/256/888/888034.png?semt=ais_hybrid'
            }
          />
          <CustomButton
            disabled={isLoading}
            title={i18n.t('signUp.btnResend')}
            onPress={reUploadFiles}
          />
        </View>
      )}
      <Modal animationType="slide" transparent={true} visible={isOpenedModal}>
        <Pressable
          onPress={() => setIsOpenedModal(false)}
          className="bg-[#00000097] h-[100%] flex justify-center items-center px-5"
        >
