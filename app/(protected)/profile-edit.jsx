...
export default function ProfileEditScreen() {
  ...
  const SchoolItem = ({ item, index }) => {
    return (
      <View className={'flex-row w-full gap-2 items-center mb-2'}>
        ...
        <View>
          <Text className={'max-h-[40px] text-[10px] px-2 bg-gray rounded-[10px] text-white'}>
            {item.school_name.replace('- ', '').trim()}
          </Text>
        </View>
        <AntDesign
          name={'delete'}
          onPress={() => handleDeleteSchool(index)}
          size={16}
          color={'black'}
        />
      </View>
  ...
  const handleDeleteSchool = (index) => {
    Alert.alert(
      i18n.t('modal.titleDeleteSchool'),
      i18n.t('modal.textDeleteSchool'),
      [
        {
  ...
  const handlerSearchSchool = async () => {
    await fetchFilterSchool({ keyword })
      .then((data) => {
        setSchools(data);
      })
      .catch((e) => {
    ...
    <View>
      ...
      >
        <View className={'h-full bg-white'}>
          ...
          <DropdownSchoolSelected
            handlerSelectSchool={handlerSelectSchool}
            selectedSchool={selectedSchool}
            schools={schools}
            handlerSearchSchool={handlerSearchSchool}
            setKeyword={setKeyword}
            keyword={keyword}
            setIsOpenDropdownSchool={setIsOpenDropdownSchool}
            isOpenDropdownSchool={isOpenDropdownSchool}
          />
