    ...
    <View className="px-4 py-3 rounded-xl border border-gray overflow-hidden">
      <View className="flex items-center justify-between flex-row">
        <TouchableOpacity onPress={toggleDropdown}>
          <Text className="max-w-[300px]">{displaySelected !== '' ? displaySelected : label}</Text>
        </TouchableOpacity>

        <TouchableOpacity onPress={toggleDropdown}>
          <Entypo
            name={visibleOptions ? 'chevron-small-up' : 'chevron-small-down'}
            size={20}
            color="black"
          />
        </TouchableOpacity>
      ...
      {visibleOptions && (
        <Animated.View style={[{ overflow: 'hidden' }]}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity onPress={() => onSelected(item.key)}>
                <View className="flex items-center justify-between flex-row mb-2">
                  <Text>{i18n.t(`subjects.${item.value}`)}</Text>

                  <Entypo
                    name={selectedIds.includes(item.key) ? 'check' : 'squared-plus'}
                    size={16}
                    color={selectedIds.includes(item.key) ? 'green' : 'gray'}
                  />
                </View>
              </TouchableOpacity>
            )}
