...
export default function NotificationCard({
  ...
  if (type === SYSTEM_MAINTENANCE) {
    ...
    const newContent =
      clearMarkdown.length > COUNT_LENGTH
        ? clearMarkdown.substring(0, COUNT_LENGTH) + '...'
        : clearMarkdown;
    return (
      <TouchableOpacity
        style={{ flex: 1, borderWidth: 2, borderColor: '#eee', borderRadius: 8 }}
        className={'h-[100px] shadow px-1 py-2 mb-4'}
        onPress={onPress}
      >
        <View className={`px-3 py-4 flex-row justify-between mb-4`}>
          <View className={'flex gap-3 flex-row'}>
            <View>
              ...
              {status === 'unread' && (
                <View
                  className={'w-[10px] absolute top-0 h-[10px] pb-2 rounded-full bg-red-600 '}
                ></View>
              )}
            </View>
    ...
    return (
      <LinearGradient
        className={'px-3 py-4 h-[180px] mb-4 shadow-2xl'}
        style={{ flex: 1, borderWidth: 2, borderColor: '#eee', borderRadius: 8 }}
        start={{ x: 0.1, y: 0.2 }}
        ...
        <View className={'flex-row justify-between'}>
          <View className={'w-[48%]'}>
            ...
            <View className={'flex flex-row gap-1 mb-2'}>
              {status === 'unread' && (
                <Text className={'text-black text-[10px] p-1 rounded bg-white'}>Chưa đọc</Text>
              )}
          ...
          <View className={'w-[48%]'}>
            {/* <Image className={"border-2 border-gray"}
                     source={{ uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQxfPq3HgXtTClXjIuCN3MUQvoekvLGyNtXmQ&s' }}
                     resizeMode={'contain'}
                     style={{
                        borderRadius: 10,
                        borderWidth: 2,
        ...
        <View className={'flex-row gap-2'}>
          <View>
            <Image
              ...
              source={require('../../assets/images/google-classroom.png')}
            />
            {status === 'unread' && (
              <View
                className={'w-[10px] absolute top-0 left-1 h-[10px] pb-2 rounded-full bg-red-600 '}
              ></View>
            )}
          </View>
      ...
      >
        <View className={'flex-row gap-2'}>
          <View>
            ...
            {status === 'unread' && (
              <View
                className={'w-[10px] absolute top-2 left-3 h-[10px] pb-2 rounded-full bg-red-600 '}
              ></View>
            )}
          </View>
