      ...
      <View className="flex flex-row">
        <View className="p-4">
          <Text className="font-semibold">
            ...
            <Text className="text-blue-600 font-semibold">{room.room_code}</Text>
          </Text>
          <Text className="text-gray text-[12px] mt-2">{createdAtConvert(room.createdAt)}</Text>
          <Text
            className={`text-gray text-[12px] mt-2 ${room.status === 'completed' ? 'text-red-500' : 'text-green-500'}`}
          >
            {room.status === 'completed'
              ? i18n.t('room_item.ended')
              : i18n.t('room_item.processing')}
          </Text>

          <Button
            text={i18n.t('room_item.viewDetail')}
