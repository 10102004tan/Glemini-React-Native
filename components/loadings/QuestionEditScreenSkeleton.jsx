  ...
  return (
    <Wrapper>
      <View className="flex flex-row items-center justify-start p-4">
        <SkeletonLoading styles=" w-[100px] h-8" />
        <SkeletonLoading styles="w-[100px] h-8 ml-3" />
      </View>
      {/* Edit View */}
        ...
        <ScrollView>
          ...
          <View className="mt-4 flex items-center justify-center flex-col">
            {Array.from({ length: 4 }).map((_, index) => (
              <SkeletonLoading key={index} styles="flex-1 h-12 w-full mb-2" />
            ))}
          </View>
          <View className="flex items-center justify-between mt-4 flex-row">
            <SkeletonLoading styles="w-[100px] h-8" />
      ...
      {/* Button */}
      <SkeletonLoading styles="p-4 h-20"></SkeletonLoading>
    </Wrapper>
  );
};
