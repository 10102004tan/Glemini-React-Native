...
const QuestionProvider = ({ children }) => {
  ...
  const deleteAnswer = (id) => {
    if (question.question_answer_ids.length > 1) {
      const newAnswers = question.question_answer_ids.filter((answer) => answer._id !== id);
  ...
  const deleteQuestion = async (quizId, questionId) => {
    const response = await fetch(`${API_URL}${API_VERSION.V1}${END_POINTS.QUESTION_DELETE}`, {
      method: 'POST',
      headers: {
  ...
  const saveQuestion = async (quizId) => {
    try {
      ...
      // Gọi API lưu câu hỏi
      const response = await fetch(`${API_URL}${API_VERSION.V1}${END_POINTS.QUESTION_CREATE}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-client-id': userData._id,
  ...
  const editQuestion = async (quizId, questionId) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}${API_VERSION.V1}${END_POINTS.QUESTION_UPDATE}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authorization: userData.accessToken,
